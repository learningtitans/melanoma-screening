...
skinScan() {
	...
	do
		echo "${path_matlab}" -nodisplay -nodesktop -nosplash -r "skinScan(${dir_dataset}/folds/${experiment}/${experiment}_train_${i}.csv, ${dir_dataset}/images/, ./features/, ./results/${experiment}_train_${i}_scores, ./results/${experiment}_train_${i}_codebook, ./results/${experiment}_train_${i}_features.svm, 'train', 6, 1); exit"  
		echo "${path_matlab}" -nodisplay -nodesktop -nosplash -r "skinScan(${dir_dataset}/folds/${experiment}/${experiment}_test_${i}.csv, ${dir_dataset}/images/, ./features/, ./results/${experiment}_train_${i}_scores, ./results/${experiment}_train_${i}_codebook, ./results/${experiment}_test_${i}_features.svm, 'test', 6, 1)); exit"       
	done
	# case "$experiment" in
