switch experiment
    ...
    case('lmplus')
    	for fold = 1:10
	        ...
	        skinScan(strcat(datasetFolder,'atlas/folds.gitnot/lmplus/lmplus_test_',int2str(fold),'.csv'), strcat(datasetFolder,'atlas/All_flat.gitnot/'), strcat(datasetFolder,'atlas/cache.gitnot/'), strcat('results.gitnot/lmplus_train_',int2str(fold),'_scores'), strcat('results.gitnot/lmplus_train_',int2str(fold),'_codebook'), strcat('results.gitnot/lmplus_test_',int2str(fold),'_features.svm'), 'test', 6)	    
	end
end
